...
use crate::{
    ...
    crypto::{is_init_message, Crypto, MessageResult},
    device::{Device, Type},
    error::Error,
    messages::{
...
impl<D: Device + Pollable, P: Protocol, S: Socket + Pollable, TS: TimeSource> GenericCloud<D, P, S, TS> {
    ...
    fn handle_device_event(&mut self, buffer: &mut MsgBuffer) {
        // HOT PATH
        try_fail!(self.device.read_msg(buffer), "Failed to read from device: {}");
    ...
    pub fn run(&mut self) {
        ...
        #[cfg(unix)]
        let waiter = try_fail!(
            WaitImpl::new(&self.socket, &self.device, 1000),
...
#[cfg(test)] use crate::device::MockDevice;
#[cfg(test)] use crate::net::MockSocket;
